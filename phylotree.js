...
d3.layout.phylotree = function (container) {
 ...
 /*--------------------------------------------------------------------------------------*/
    ...
    phylotree.placenodes = function () {
        ...
        var extent = d3.extent(nodes.map(function (d) { return node_span (d); }));
        min_node_span = extent[0];
        max_node_span = extent[1];
        node_span_scale.domain(extent);
  ...
  phylotree.update = function (transitions) {
      ...
      drawn_nodes.attr("transform", function(d) {
          d.screen_x = x_coord(d) + phylotree.node_bubble_size(d);
